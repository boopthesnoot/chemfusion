...
class GraphStringDataset(dgl.data.DGLDataset, RegPropDataset):
    ...
    def process(self):
        ...
        assert self.task in ["regression", "reconstruction"]
        assert self.sample_size is None or (
            self.sample_size > 0
            and (
                type(self.sample_size) is int
                or (type(self.sample_size) is float and self.sample_size < 1)
            )
        )
        ...
        edges = pd.read_csv(os.path.join(data_path, f"{self.sample_type}_edges.csv"))
        nodes = pd.read_csv(os.path.join(data_path, f"{self.sample_type}_nodes.csv"))
        targets = pd.read_csv(os.path.join(data_path, f"{self.sample_type}_target.csv"))
        self.mol_id_list = np.sort(np.unique(edges["mol_id"].values))
        if self.sample_size is not None:
            if type(self.sample_size is int):
                sample_size = min(self.sample_size, len(self.mol_id_list))
            else:
                sample_size = int(self.sample_size * len(self.mol_id_list))
            self.mol_id_list = np.sort(
                np.random.choice(self.mol_id_list, size=sample_size, replace=False)
            )
            edges = edges.loc[edges["mol_id"].isin(self.mol_id_list)]
            nodes = nodes.loc[nodes["mol_id"].isin(self.mol_id_list)]
            targets = targets.loc[targets["mol_id"].isin(self.mol_id_list)]

        chosen_cols = nodes.columns[~nodes.columns.isin(("mol_id", "node_id"))]
        grouped_edges = edges.groupby("mol_id")
        grouped_nodes = nodes.groupby("mol_id")
        grouped_targets = targets.groupby("mol_id")
        for (
            (mol_num1, group_edges),
            (mol_num2, group_nodes),
            (mol_num3, group_target),
        ) in tqdm(
            zip(grouped_edges, grouped_nodes, grouped_targets),
            total=len(self.mol_id_list),
        ):
            assert mol_num1 == mol_num2 == mol_num3
            src = torch.from_numpy(group_edges["src"].to_numpy())
            trg = torch.from_numpy(group_edges["trg"].to_numpy())
            edge_features = torch.from_numpy(group_edges["type"].to_numpy())

            node_features = group_nodes[chosen_cols].to_numpy(dtype=int)
            graph = dgl.graph((src, trg))
            graph.edata["edge_feats"] = edge_features.to(torch.float32)
            graph.ndata["node_feats"] = torch.from_numpy(node_features).to(
                torch.float32
            )
            self.graphs.append(graph)
            target = group_target.to_numpy()[0]
            self.targets.append(target[1])

        self.targets = torch.from_numpy(np.array(self.targets, dtype=np.float32))
        self.graph_dims = {
            "nodes": self.graphs[0].ndata["node_feats"].shape[1],
            "edges": max(
                [
                    self.graphs[i].edata["edge_feats"].shape[0]
    ...
    def __getitem__(self, i):
        ...
        if self.task == "reconstruction":
            ...
            assert target == self.targets[i]
        return self.graphs[i], self.smiles[i], self.targets[i]
...
class StringDataset(RegPropDataset):
    def __init__(self, cfg, sample_type, task, sample_size=None):
        ...
        assert sample_size is None or (
            sample_size > 0
            and (
                type(sample_size) is int
                or (type(sample_size) is float and sample_size < 1)
            )
        )
        pad_symbol = cfg.general.pad_symbol
        data_path = os.path.join(cfg.general.proj_path, "data")
        self.info = {}
        regr_info = joblib.load(
            os.path.join(data_path, cfg.run.regr_dataset, "string_preproc", "info.pkl")
        )
        reco_info = joblib.load(
            os.path.join(data_path, cfg.run.reco_dataset, "string_preproc", "info.pkl")
        )
        self.info["max_len"] = max(reco_info["max_len"], reco_info["max_len"])
        ...
        trg_data = torch.Tensor(
            pd.read_csv(
                os.path.join(
                    ...
                    "string_preproc",
                    f"{sample_type}_target.csv",
                ),
                index_col="mol_id",
            ).values
        )
        self.targets = trg_data
        dummy_graph = dgl.graph((torch.tensor([0]), torch.tensor([0])), num_nodes=1)
        dummy_graph.edata["edge_feats"] = torch.from_numpy(np.array([1])).to(
            torch.float32
        )
        dummy_graph.ndata["node_feats"] = torch.from_numpy(np.array([1])).to(
            torch.float32
        )
        self.graphs = [dummy_graph] * len(self.targets)

        if sample_size is not None:
            if type(sample_size) is int:
                sample_size = min(sample_size, len(self.smiles))
            else:
        ...
        self.targets = torch.from_numpy(np.array(self.targets)).to(torch.float32)
