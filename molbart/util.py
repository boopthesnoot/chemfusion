...
def calc_train_steps(cfg, dm):
    dm.setup()
    batches_per_gpu = math.ceil(len(dm.train_dataloader()) / float(cfg.general.gpus))
    train_steps = (
        math.ceil(batches_per_gpu / cfg.regression.acc_batches) * cfg.regression.epochs
    )
    return train_steps
